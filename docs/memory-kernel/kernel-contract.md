...
|---|---|---|---|
| **1. Hot Context** | `WakeBuilder.build()`, `ActiveMemoryEngine.build()`, profiles (default/casual/coding/writing/privacy), project resolution, compiled cards (L0), recent sessions, pinned decisions. Active memory responds with block/summary/sources/warnings. | Wake and active-memory do not share one typed internal packet. Active memory owns too many concerns (policy/retrieval/composition/rendering). No `EntityContext` pre-flight. Response shapes overlap but are not unified. | Slice 3 (extraction) + Slice 5 (entity context) + Slice 8 (shared packet) |
| **2. Entity Memory** | `EntityRegistry` (SQLite), `SubjectResolver` (deterministic + LLM), semantic write routing, compiled wiki cards, `ClaimStore` for active durable truth. | No small entity-context packet for retrieval. Relationships are not clearly separated from links/aliases. No observation index. Entity lookup is per-call only; no batch entity context. | Slice 5 (entity context packet) + Slice 6 (observation index) |
| **3. Evidence Retrieval** | Exact `get` (file read), BM25/vector/hybrid search, session FTS5 search, claim/event/evidence lookup, link neighbors, retrieval planner (optional LLM), reranker (optional LLM), session fallback. | Planner emits query strings, not typed retrieval attempts. Search engine owns too many stages (query → FTS → vector → session → rerank → finalize) in one class. No typed retrieval-attempt protocol. | Slice 7 (typed retrieval attempts), search-engine cleanup backlog |
| **4. Compiler** | Dream (extraction/recall/proposals/events), daily/weekly digests, wiki refresh (`WikiIndexBuilder`), compiled wiki card refresh, maintenance reports, migration audit/repair, session ingest + recall promotion. | Not described as one compiler plane. Extraction/promotion boundaries are uneven. No unified compiler contract (input → derived → reviewable → canonical). | Slice 9 (compiler plane reconciliation) |
