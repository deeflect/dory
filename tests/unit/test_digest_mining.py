...
def test_extractor_parses_valid_payload() -> None:
    client = _FakeClient(
        payload={
            "claims": [
                {
                    "subject_ref": "project:sample-spec",
                    "kind": "state",
                    "statement": "Sample moved from spec to build",
                    "confidence": "high",
                    "time_ref": "2026-04-10",
    ...
    assert len(claims) == 2
    assert claims[0].subject_ref == "project:sample-spec"
...
def test_mine_digest_file_stores_claims(tmp_path: Path) -> None:
    ...
    extractor = _StubExtractor(
        claims=[
            DigestClaim(
                ...
                kind="state",
                statement="Atlas is active focus",
                confidence="high",
                time_ref="2026-04-10",
                evidence_path="logs/daily/2026-04-10.md",
                reason=None,
    ...
    claims = store.current_claims("project:atlas")
    assert len(claims) == 1
    assert claims[0].statement == "Atlas is active focus"
