...
def test_merge_with_entity_appends_backlink(tmp_path: Path) -> None:
    corpus = tmp_path / "corpus"
    _write_idea(corpus, "atlas-pricing-thoughts", "just notes about atlas pricing\n")
    client = _FakeClient(
        payloads=[
            {
                "decisions": [
                    {
                        "source_path": "ideas/atlas-pricing-thoughts.md",
                        "classification": "merge_with_entity",
                        "target_slug": "atlas",
                        "rationale": "direct pricing evidence for atlas",
                    }
                ]
            }
        ]
    ...
    report = promote_ideas(corpus, [_entity("atlas")], client=client)  # type: ignore[arg-type]

    assert report.merged_into_entity == 1
    text = (corpus / "ideas" / "atlas-pricing-thoughts.md").read_text(encoding="utf-8")
    assert "<!-- merged-with: atlas -->" in text
