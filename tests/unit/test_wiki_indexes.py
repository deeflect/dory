...
def test_wiki_index_builder_writes_global_and_family_indexes(tmp_path: Path) -> None:
    ...
    (wiki_root / "projects" / "sample.md").write_text(
        "---\n"
        "title: Sample\n"
        "type: wiki\n"
        "status: active\n"
    ...
    (wiki_root / "people" / "avery.md").write_text(
        "---\n"
        "title: Avery\n"
        "type: wiki\n"
        "status: active\n"
    ...
    assert "[[wiki/projects/sample|Sample]]" in projects_index
...
def test_wiki_hot_page_prefers_claim_event_facts_when_claim_store_exists(tmp_path: Path) -> None:
    ...
    (wiki_root / "sample.md").write_text(
        "---\n"
        "title: Sample\n"
        "type: wiki\n"
        "status: active\n"
    ...
    hot_page = (tmp_path / "wiki" / "hot.md").read_text(encoding="utf-8")
    assert "## Key Recent Facts" in hot_page
    assert "- Sample is the active focus this week." in hot_page
    assert "## Recent Changes" in hot_page
    assert "added Sample is the active focus this week." in hot_page

    log_page = (tmp_path / "wiki" / "log.md").read_text(encoding="utf-8")
    assert "## Recent Claim Changes" in log_page
    assert "added Sample is the active focus this week." in log_page
...
def test_memory_health_dashboard_requires_real_evidence_refs(tmp_path: Path) -> None:
    ...
    assert report["missing_evidence"] == ["wiki/projects/sample.md"]
...
def test_memory_health_dashboard_flags_missing_timeline_for_claim_style_page(tmp_path: Path) -> None:
    ...
    assert report["missing_timeline"] == ["wiki/projects/sample.md"]
...
def test_memory_health_dashboard_ignores_placeholder_contradiction_lines(tmp_path: Path) -> None:
    ...
    (wiki_root / "sample.md").write_text(
        ...
        "- sources/semantic/2026/04/14/sample-write.md\n\n"
        "## Timeline\n"
        "- 2026-04-14T00:00:00Z: Sample is active.\n\n"
        "## Contradictions\n"
...
def test_memory_health_dashboard_flags_event_mismatch_between_timeline_and_evidence(tmp_path: Path) -> None:
    ...
    (wiki_root / "sample.md").write_text(
        ...
        "- sources/semantic/2026/04/14/sample-write.md\n\n"
        "## Timeline\n"
        "- 2026-04-14T00:00:00Z: Retired: Sample is active.\n",
        encoding="utf-8",
    )

    report = MemoryHealthDashboard(tmp_path).inspect()

    assert report["event_mismatch"] == ["wiki/projects/sample.md"]
...
def test_memory_health_dashboard_flags_state_conflict_when_only_retired_events_exist(tmp_path: Path) -> None:
    ...
    assert report["state_conflict"] == ["wiki/projects/sample.md"]
...
def test_memory_health_dashboard_flags_claim_mismatch_against_claim_store(tmp_path: Path) -> None:
    ...
    (wiki_root / "sample.md").write_text(
        ...
        "- sources/semantic/2026/04/14/sample-write.md\n\n"
        "## Timeline\n"
        "- 2026-04-14T00:00:00Z: Sample is paused.\n",
        encoding="utf-8",
    )

    report = MemoryHealthDashboard(tmp_path).inspect()

    assert report["claim_mismatch"] == ["wiki/projects/sample.md"]
...
def test_memory_health_dashboard_flags_claim_event_and_evidence_mismatch(tmp_path: Path) -> None:
    ...
    store.retire_entity_claims(
        ...
        evidence_path="sources/semantic/2026/04/14/sample-forget.md",
    )
    wiki_root = tmp_path / "wiki" / "projects"
    wiki_root.mkdir(parents=True)
    (wiki_root / "sample.md").write_text(
        ...
        "- sources/semantic/2026/04/14/sample-write.md\n\n"
        "## Timeline\n"
        "- 2026-04-14T00:00:00Z: Added: Sample is the active focus this week.\n",
        encoding="utf-8",
    )

    report = MemoryHealthDashboard(tmp_path).inspect()

    assert report["claim_event_mismatch"] == ["wiki/projects/sample.md"]
    assert report["claim_evidence_mismatch"] == ["wiki/projects/sample.md"]
