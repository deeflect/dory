...
def test_claude_projects_collector_collects_main_session_text(tmp_path: Path) -> None:
                ...
                json.dumps(
                    {
                        ...
                        "message": {
                            ...
                            "content": [
                                ...
                                {"type": "text", "text": "Sample is the active focus this week."},
    ...
    assert "Sample is the active focus this week." in capture.raw_text
...
def test_openclaw_sessions_collector_reads_agent_session_jsonl(tmp_path: Path) -> None:
                ...
                json.dumps(
                    {
                        ...
                        "role": "assistant",
                        "content": [{"type": "text", "text": "Atlas stays on the small Hetzner VPS."}],
                    }
                ),
            ]
        )
    ...
    assert "Atlas stays on the small Hetzner VPS." in capture.raw_text
...
def test_hermes_sessions_collector_reads_transcript_jsonl(tmp_path: Path) -> None:
    ...
    assert "what did we decide for Sample?" in capture.raw_text
    assert "Sample is the active focus this week." in capture.raw_text
    assert "reasoning" not in capture.raw_text
