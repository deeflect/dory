...
def test_concept_kind_for_legacy_path() -> None:
    ...
    assert concept_kind_for_legacy_path("memory/projects/sample-spec.md") == "general"
...
def test_render_canonical_template_has_required_sections() -> None:
    ...
    assert doc.frontmatter["title"] == "Sample"
    assert doc.frontmatter["type"] == "project"
    assert doc.frontmatter["slug"] == "sample"
...
def test_normalize_classification_target_forces_supported_core_and_canonical_paths() -> None:
    core = ClassifiedDocument(
        ...
        target_path="core/nimbus_persona.md",
        domain="mixed",
        entity_refs=(),
        decision_refs=(),
        time_scope="current",
    ...
    project = ClassifiedDocument(
        ...
        canonicality="canonical",
        target_path="notes/sample plan.md",
        domain="work",
    ...
    assert normalize_classification_target(project).target_path == "projects/sample/state.md"
