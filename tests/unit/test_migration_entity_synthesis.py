...
def _synthesis_payload(
    *,
    ...
    return {
        ...
        "aliases": aliases or ["atlas", "Atlaz"],
        "evidence_cited": evidence or ["projects/atlas/state.md"],
    }
...
def test_synthesize_merges_active_and_archive_as_evidence(tmp_path: Path) -> None:
    corpus = tmp_path / "corpus"
    _write(corpus / "projects" / "nimbus-bot" / "state.md", "active content")
    _write(corpus / "archive" / "projects" / "nimbus-bot.md", "archived content")
    client = _FakeClient(
        payloads=[
            _synthesis_payload(
                title="nimbus-bot",
                evidence=["projects/nimbus-bot/state.md", "archive/projects/nimbus-bot.md"],
            )
        ]
    )
    entities = [
        _entity(
            "nimbus-bot",
            evidence_paths=("projects/nimbus-bot/state.md", "archive/projects/nimbus-bot.md"),
        )
    ]
    ...
    assert report.synthesized == 1
    doc = load_markdown_document((corpus / "projects" / "nimbus-bot" / "state.md").read_text(encoding="utf-8"))
    assert "projects/nimbus-bot/state.md" in doc.body
    assert "archive/projects/nimbus-bot.md" in doc.body
        ...
        "entities": [
            {
                ...
                "status_signal": "active",
                "evidence_paths": ["projects/atlas/state.md"],
                "mention_count": 3,
            }
        ]
    }
