...
def test_claim_store_records_event_evidence_path(tmp_path) -> None:
    ...
    assert events[0].entity_id == "person:avery"
    assert events[0].event_type == "added"
    assert events[0].evidence_path == "sources/semantic/2026/04/14/avery-write.md"
...
def test_claim_store_replace_records_replaced_and_added_events(tmp_path) -> None:
    ...
    assert events[1].evidence_path == "sources/semantic/2026/04/15/avery-replace.md"
    assert events[2].evidence_path == "sources/semantic/2026/04/15/avery-replace.md"
...
def test_claim_store_replace_can_preserve_source_time_for_events(tmp_path) -> None:
    ...
    current = store.current_claims("project:sample", kind="state")
    history = store.claim_history("project:sample")
    events = store.claim_events("project:sample")
