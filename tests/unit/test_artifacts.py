...
def test_resolve_artifact_target_routes_report() -> None:
    req = ArtifactReq(
        kind="report",
        title="Sample focus status",
        question="What are we working on right now?",
        body="Sample is the active focus this week.",
        sources=["core/active.md"],
    )

    target = resolve_artifact_target(req, created="2026-04-13")

    assert target == "references/reports/2026-04-13-sample-focus-status.md"
...
def test_resolve_artifact_target_routes_briefing() -> None:
    ...
    assert target == "references/briefings/2026-04-13-sample-briefing.md"
...
def test_render_report_artifact_includes_question_and_sources() -> None:
    req = ArtifactReq(
        ...
        question="What are we working on right now?",
        body="Sample is the active focus this week.",
        sources=["core/active.md", "wiki/projects/sample.md"],
    )
    ...
    assert "- wiki/projects/sample.md" in rendered
...
def test_render_wiki_note_artifact_uses_wiki_note_sections() -> None:
    req = ArtifactReq(
        kind="wiki-note",
        title="Sample topic",
        question="What do we know about Sample?",
        body="Sample is the active focus this week.",
        sources=["core/active.md"],
    )
...
def test_artifact_writer_persists_markdown(tmp_path: Path) -> None:
    ...
    assert resp.path == "wiki/concepts/sample-topic.md"
    written = (tmp_path / resp.path).read_text(encoding="utf-8")
    assert "# Sample topic" in written
