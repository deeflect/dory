...
def test_active_project_top_level_becomes_state_file(tmp_path: Path) -> None:
    decision = _route(tmp_path, "active/projects/nimbus-bot.md")

    assert decision.kind == "route"
    assert decision.destination == Path("projects/nimbus-bot/state.md")
...
def test_active_project_supporting_flat_routes_to_supporting(tmp_path: Path) -> None:
    decision = _route(tmp_path, "active/projects/_supporting/nimbus-bot.md")

    assert decision.kind == "route"
    assert decision.destination == Path("projects/nimbus-bot/supporting.md")
...
def test_media_directory_excluded(tmp_path: Path) -> None:
    decision = _route(tmp_path, "media/images/nimbus.png")
