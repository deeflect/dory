...
def test_record_recall_event_persists_selected_path(tmp_path: Path) -> None:
    ...
    req = RecallEventReq(
        ...
        session_key="sess-1",
        query="who is avery",
        result_paths=["people/avery.md", "core/user.md"],
    ...
    assert resp.selected_path == "people/avery.md"
    ...
    assert len(recent) == 1
    assert recent[0].selected_path == "people/avery.md"
    assert recent[0].result_paths == ("people/avery.md", "core/user.md")
    ...
    assert diagnostics.last_recall_selected_path == "people/avery.md"


def test_recall_promotion_candidates_and_marks_are_persisted(tmp_path: Path) -> None:
    store = OpenClawParityStore(index_root=tmp_path / ".index")
    for query in ("who is avery", "avery prefs", "avery status"):
    ...
    assert candidate.selected_path == "people/avery.md"
    assert candidate.event_count == 3
    assert candidate.query_count == 3
    assert "who is avery" in candidate.sample_queries

    store.mark_recall_promotion(candidate=candidate, distilled_path="inbox/distilled/recall-people-avery.md")
...
def test_list_public_artifacts_returns_expected_paths_and_metadata(tmp_path: Path) -> None:
    ...
    (corpus_root / "references" / "reports" / "migrations" / "2026-04-13-sample.md").write_text(
        dump_markdown_document(
            {"title": "Sample Migration", "agent": "openclaw"},
            "Migration report.\n",
        ),
        encoding="utf-8",
    )
    (corpus_root / "references" / "briefings" / "2026-04-13-sample.md").write_text(
        dump_markdown_document(
    ...
    (corpus_root / "wiki" / "projects" / "sample.md").write_text(
        dump_markdown_document(
            {"title": "Sample Wiki"},
            "Wiki.\n",
        ),
    ...
    assert [artifact.relative_path for artifact in artifacts] == [
        "core/user.md",
        "references/briefings/2026-04-13-sample.md",
        "references/reports/migrations/2026-04-13-sample.md",
        "wiki/projects/sample.md",
    ]
