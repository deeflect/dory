    ...
    def plan_search(self, *, query: str, corpus: str):
        ...
        if "Atlaz" in query:
            return SearchRetrievalPlan(
                durable_queries=(query, "Atlas pricing Hetzner"),
                session_queries=(),
...
def test_hybrid_uses_expanded_queries_for_bm25_recall(monkeypatch, tmp_path: Path) -> None:
    ...
    rows = {
        "atlas": _ChunkRow(
            chunk_id="atlas",
            path="projects/atlas/state.md",
            content="Pricing: $19/mo BYOK. Hetzner CX22.",
            start_line=1,
            end_line=4,
            frontmatter_json='{"title":"atlas","type":"project","status":"active","canonical":true,"source_kind":"human","temperature":"warm"}',
            score=0.0,
        ),
        "studio": _ChunkRow(
            chunk_id="studio",
    ...
    def fake_bm25(query: str, limit: int) -> list[_ChunkRow]:
        ...
        if "Atlas pricing Hetzner" in query:
            return [rows["atlas"]]
        return [rows["studio"]]
    ...
    response = engine.search(SearchReq(query="What's the pricing plan for Atlaz?", mode="hybrid", k=2))

    assert "projects/atlas/state.md" in {row.path for row in response.results}
    assert seen_queries == ["What's the pricing plan for Atlaz?", "Atlas pricing Hetzner"]
...
def test_search_can_reorder_results_with_result_selector(monkeypatch, tmp_path: Path) -> None:
    ...
    response = engine.search(
        SearchReq(query="What's the pricing plan for Atlaz?", mode="hybrid", corpus="durable", k=2)
    )
...
def test_hybrid_uses_expanded_queries_for_vector_recall(monkeypatch, tmp_path: Path) -> None:
    ...
    ranked = engine._hybrid("What's the pricing plan for Atlaz?", 2, warnings=[])

    assert "projects/atlas/state.md" in {row.path for row in ranked}
    assert seen_vector_queries == ["What's the pricing plan for Atlaz?", "Atlas pricing Hetzner"]
...
def test_hybrid_skips_expansion_for_current_state_queries(monkeypatch, tmp_path: Path) -> None:
    ...
    ranked = engine._hybrid("What model is nimbus's default right now?", 2, warnings=[])

    assert ranked[0].path == "core/env.md"
    assert seen_queries == ["What model is nimbus's default right now?"]
        ...
        lambda req, started, rerank_enabled, warnings, search_plan=None: type(
            ...
            (),
            {
                ...
                "results": [
                    ...
                    SearchResult(
                        path=rows["atlas"].path,
                        lines="1-4",
                        snippet="Pricing: $19/mo BYOK. Hetzner CX22.",
                        score=rows["atlas"].score,
                        frontmatter={},
                        stale_warning=None,
                    ),
                ],
...
def test_hybrid_prefers_canonical_project_state_over_support_notes(monkeypatch, tmp_path: Path) -> None:
    ...
    rows = {
        "state": _ChunkRow(
            chunk_id="state",
            path="projects/trawler/state.md",
        ...
        "support": _ChunkRow(
            chunk_id="support",
            path="projects/trawler/notes-import/trawler.md",
            content="Trawler archived notes.",
            start_line=1,
            end_line=4,
    ...
    ranked = engine._hybrid("What stack and API did we pick for Trawler?", 2, warnings=[])

    assert ranked[0].path == "projects/trawler/state.md"
...
def test_hybrid_recovers_close_identifier_renames(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(engine, "_bm25", lambda query, limit: [rows["studio"], rows["atlas"]])
    monkeypatch.setattr(engine, "_vector", lambda query, limit: [rows["studio"], rows["atlas"]])
    monkeypatch.setattr(
        engine,
        "_rows_for_chunk_ids",
        lambda chunk_ids, score_map=None: [
    ...
    ranked = engine._hybrid("What's the pricing plan for Atlaz and what VPS is it meant to run on?", 2, warnings=[])

    assert ranked[0].path == "projects/atlas/state.md"
