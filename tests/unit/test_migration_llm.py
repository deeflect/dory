...
def test_parse_classification_response() -> None:
    ...
    assert parsed.target_path == "projects/sample/state.md"
    assert parsed.entity_refs == ("project:sample",)
...
def test_parse_extraction_response_can_salvage_valid_atoms_with_context() -> None:
    ...
    assert len(atoms) == 1
    assert atoms[0].subject_ref == "project:sample"
...
def test_migration_llm_extracts_document_with_resolution_mode() -> None:
            ...
            "atoms": [
                {
                    ...
                    "subject_ref": "project:sample",
                    "payload": {"summary": "Sample is active again."},
                    "evidence_path": "projects/sample/state.md",
                    "time_ref": "2026-04-13",
                    "confidence": "high",
                }
            ],
    ...
    extracted = engine.extract_document(path="memory/projects/sample.md", text="# Sample")

    assert extracted.resolution_mode == "resolved"
    assert extracted.classified.target_path == "projects/sample/state.md"
