...
def test_query_expander_filters_duplicates() -> None:
    class FakeClient:
        def generate_json(self, **kwargs):
            return {"expansions": ["Atlas pricing Hetzner", "atlas pricing hetzner", ""]}

    expander = OpenRouterQueryExpander(client=FakeClient(), max_expansions=3)  # type: ignore[arg-type]

    expanded = expander.expand("Atlaz pricing")

    assert expanded == ["Atlas pricing Hetzner"]


def test_query_expander_returns_empty_on_non_schema_payload() -> None:
    class FakeClient:
        def generate_json(self, **kwargs):
            return {"content": "- Atlas pricing Hetzner\n- Atlas CX22"}
...
def test_retrieval_planner_only_includes_session_results_for_all_corpus() -> None:
    class FakeClient:
        def generate_json(self, **kwargs):
            return {
                "durable_queries": ["Sample active focus"],
    ...
    assert all_plan.session_queries == ("what are we working on today", "Sample follow-up")
