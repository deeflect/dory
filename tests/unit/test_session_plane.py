...
def test_session_plane_upserts_and_searches_recent_entries(tmp_path: Path) -> None:
    ...
    plane.upsert_session_chunk(
        ...
        content="We decided to use Sample as the active focus this week.",
    ...
    result = plane.search(SessionSearchQuery(query="active focus Sample", limit=5))

    assert result.count == 1
    assert result.results[0].path == "logs/sessions/claude/macbook/2026-04-12-s1.md"
    assert "Sample" in result.results[0].snippet
...
def test_session_plane_updates_search_index_without_full_rebuild(tmp_path: Path) -> None:
    ...
    plane.upsert_session_chunk(
        path=path,
        content="We are focused on Sample this week.",
        updated="2026-04-12T10:00:00Z",
        agent="claude",
        device="macbook",
        session_id="s1",
    ...
    first = plane.search(SessionSearchQuery(query="Sample", limit=5))
    ...
    stale = plane.search(SessionSearchQuery(query="Sample", limit=5))
