...
def test_render_compiled_page_from_claim_records() -> None:
    markdown = render_compiled_page_from_claim_records(
        title="Avery",
        summary="Avery prefers written updates.",
        claim_records=(
            ClaimRecord(
                claim_id="claim-1",
    ...
    assert "Avery prefers written updates." in markdown
    assert "### Added" in markdown
    assert "sources/semantic/2026/04/15/avery-replace.md - Avery prefers written updates." in markdown
    assert "2026-04-15T00:00:00Z: Avery prefers written updates." in markdown
    assert "people/avery.md" not in markdown
...
def test_render_compiled_page_timeline_and_evidence_include_event_types() -> None:
    markdown = render_compiled_page(
        ...
        claim_events=(
            ClaimEvent(
                ...
                event_type="added",
                reason=None,
                evidence_path="sources/semantic/2026/04/15/sample-write.md",
                created_at="2026-04-15T00:00:00Z",
            ),
            ClaimEvent(
                event_id="event-2",
                ...
                event_type="retired",
                reason="superseded",
                evidence_path="sources/semantic/2026/04/16/sample-forget.md",
    ...
    assert "sources/semantic/2026/04/15/sample-write.md - Sample is the active focus this week." in markdown
    assert "2026-04-16T00:00:00Z: Retired: Sample is the active focus this week. (superseded)" in markdown


def test_run_compiled_wiki_refresh_uses_claim_store_records_and_events(tmp_path: Path) -> None:
    source = tmp_path / "people" / "avery.md"
...
""",
    ...
    store.add_claim(
        entity_id="person:avery",
        kind="timezone",
        statement="Avery is based in Berlin.",
        evidence_path="sources/semantic/2026/04/15/avery-berlin.md",
        occurred_at="2026-04-15",
    )
    store.replace_current_claim(
        entity_id="person:avery",
        kind="preference",
        statement="Avery prefers concise written updates.",
        evidence_path="sources/semantic/2026/04/16/avery-concise.md",
    ...
    assert "wiki/people/avery.md" in written
    content = (tmp_path / "wiki" / "people" / "avery.md").read_text(encoding="utf-8")
    assert "Avery prefers concise written updates. [active, high, fresh]" in content
    assert "Avery is based in Berlin. [active, high, fresh]" in content
...
""",
    ...
    stale_page = wiki_people / "former-avery.md"
...
""",
    ...
    assert (tmp_path / "wiki" / "people" / "avery.md").exists()
