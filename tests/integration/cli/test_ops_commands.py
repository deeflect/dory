...
def test_ops_dream_once_promotes_recall_candidates(cli_runner, monkeypatch, tmp_path: Path) -> None:
    ...
    (corpus_root / "people" / "avery.md").write_text("# Avery\n", encoding="utf-8")
    ...
    assert result.exit_code == 0, result.output
    payload = json.loads(result.stdout)
    assert payload["distilled"] == ["inbox/distilled/recall-people-avery.md"]
    assert payload["proposed"] == ["inbox/proposed/recall-people-avery.json"]
...
""",
    ...
    assert result.exit_code == 0, result.output
    payload = json.loads(result.stdout)
    assert "wiki/projects/sample.md" in payload["written"]
