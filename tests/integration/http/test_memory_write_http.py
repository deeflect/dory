...
def test_http_memory_write_handles_write_replace_and_forget(
    ...
    assert replace_payload["target_path"] == "projects/sample/state.md"
    project_document = load_markdown_document(
        (corpus_root / "projects" / "sample" / "state.md").read_text(encoding="utf-8")
    )
...
def test_http_memory_write_returns_400_for_validation_errors(
    ...
    assert error["code"] == "dory_validation_error"
    assert error["message"] == "semantic write failed"
