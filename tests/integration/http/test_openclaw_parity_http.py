...
def _seed_public_artifacts(root: Path) -> None:
    ...
    (root / "references" / "reports" / "migrations" / "2026-04-13-sample.md").write_text(
        "---\ntitle: Sample Migration\nagent: openclaw\n---\n# Sample Migration\n",
        encoding="utf-8",
    )
    (root / "wiki" / "projects" / "sample.md").write_text(
        "---\ntitle: Sample\n---\n# Sample\n",
        encoding="utf-8",
    )
...
def test_http_recall_event_and_public_artifacts_and_status(tmp_path: Path) -> None:
    ...
    assert recall.json()["selected_path"] == "people/avery.md"
...
def test_http_search_respects_min_score_for_openclaw_clients(tmp_path: Path, fake_embedder) -> None:
    ...
    (corpus_root / "people" / "avery.md").write_text(
        "---\n"
        "title: Avery\n"
        "type: person\n"
        "status: active\n"
    ...
    baseline = client.post(
        "/v1/search",
        json={"query": "Avery prefers async work", "mode": "hybrid", "k": 5},
    )
