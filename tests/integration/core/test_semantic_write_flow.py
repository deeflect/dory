...
def test_semantic_write_flow_handles_write_replace_and_forget(tmp_path: Path) -> None:
    ...
    assert "Sample is paused." in project_document.body
    assert "Sample is active." not in project_document.body
...
def test_semantic_write_flow_creates_structured_decision_pages(tmp_path: Path) -> None:
    ...
    assert response.target_path == "decisions/sample.md"
    decision = load_markdown_document((root / "decisions" / "sample.md").read_text(encoding="utf-8"))
    assert decision.frontmatter["type"] == "decision"
    assert "## Decision" in decision.body
    assert "Sample is the active focus this week." in decision.body
