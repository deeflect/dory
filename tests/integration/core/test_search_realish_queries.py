...
def test_search_prefers_atlas_project_notes_for_atlaz_pricing_query(
    tmp_path: Path,
    fake_embedder,
) -> None:
    corpus_root = tmp_path / "corpus"
    ...
    state_path = corpus_root / "projects" / "atlas" / "state.md"
...
""",
    ...
    notes_path = corpus_root / "projects" / "atlas" / "notes-from-daily-digests.md"
    notes_path.write_text(
        """---
title: atlaz — notes from daily digests
created: 2026-04-07
type: project
status: active
canonical: false
...
- Atlaz pricing: $19/mo BYOK, $49/mo Standard, $99/mo Pro.
...
""",
    ...
    result = engine.search(
        SearchReq(
            query="What's the pricing plan for Atlaz and what VPS is it meant to run on?",
            mode="hybrid",
            k=3,
        )
    )

    assert result.results[0].path == "projects/atlas/notes-from-daily-digests.md"
...
def test_search_prefers_temporal_digest_for_soul_cleanup_query(
    ...
    digest_path.write_text(
        """---
title: Mon (02/09) — Trawler plugin, Plugin routing
created: 2026-02-09
type: daily
date: 2026-02-09
status: done
