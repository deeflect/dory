...
def test_migration_bootstraps_canonical_pages(tmp_path: Path) -> None:
    ...
    assert not (output_root / "projects" / "sample" / "state.md").exists()
    ...
    assert (output_root / "sources" / "imported" / "projects" / "sample-spec.md").exists()
...
def test_migration_preserves_evidence_without_heuristic_canonical_promotion(tmp_path: Path) -> None:
    ...
    project_source = (output_root / "sources" / "imported" / "projects" / "sample-spec.md").read_text(encoding="utf-8")
    assert "Sample" in project_source
    assert not (output_root / "projects" / "sample" / "state.md").exists()
    assert not (output_root / "concepts" / "openclaw.md").exists()
...
def test_migration_engine_stages_jsonl_transcripts_as_markdown_evidence(tmp_path: Path) -> None:
                ...
                json.dumps(
                    {
                        ...
                        "role": "user",
                        "content": [{"type": "text", "text": "what did we decide for Sample?"}],
                    }
                ),
                json.dumps(
                    {
                        ...
                        "role": "assistant",
                        "content": [{"type": "text", "text": "Sample is the active focus this week."}],
                    }
                ),
            ]
        )
    ...
    assert "- user: what did we decide for Sample?" in document.body
    assert "- assistant: Sample is the active focus this week." in document.body
...
def test_migration_engine_stages_ndjson_inputs_as_markdown_evidence(tmp_path: Path) -> None:
    ...
    transcript.write_text(
        "\n".join(
            [
                ...
                json.dumps({"timestamp": "2026-04-12T10:00:03Z", "role": "assistant", "content": "Atlas stays on the small Hetzner VPS."}),
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    assert "Atlas stays on the small Hetzner VPS." in body
...
def test_migration_engine_promotes_deterministic_transcript_atoms_into_canonical_state(tmp_path: Path) -> None:
    ...
    project_state = output_root / "projects" / "sample" / "state.md"
    ...
    assert "Sample is the active focus this week." in project_state.read_text(encoding="utf-8")
    ...
    assert payload["atoms"] == [
        {
            ...
            "kind": "project_update",
            "payload": {
                "summary": "Sample is the active focus this week.",
                "title": "Sample",
            },
...
def test_migration_engine_uses_llm_when_available(tmp_path: Path) -> None:
    ...
    legacy_root.mkdir()
    note = legacy_root / "notes.md"
    note.write_text("# Profile\n\nJordan prefers async work and Sample is active.\n", encoding="utf-8")
...
def test_migration_document_artifact_records_llm_fallback_reason(tmp_path: Path) -> None:
    ...
    note.write_text("Sample is active.\n", encoding="utf-8")
...
def test_migration_engine_salvages_valid_llm_atoms_when_some_are_invalid(tmp_path: Path) -> None:
    ...
    note.write_text("Sample is active again.\n", encoding="utf-8")
                    ...
                    "atoms": [
                        {
                            "kind": "project_update",
                            "subject_ref": "sample",
                            "payload": {"summary": "Sample is active again."},
    ...
    project_state = output_root / "projects" / "sample" / "state.md"
    assert project_state.exists()
    assert "Sample is active again." in project_state.read_text(encoding="utf-8")
...
def test_migration_engine_persists_contradictions_in_run_artifact(tmp_path: Path) -> None:
    ...
    archive_note = legacy_root / "memory" / "archive" / "projects" / "sample-old.md"
    daily_note = legacy_root / "memory" / "daily" / "2026-04-13-digest.md"
    archive_note.write_text("Sample was parked.\n", encoding="utf-8")
    daily_note.write_text("Sample is the active focus.\n", encoding="utf-8")
        ...
        def generate_json(self, *, schema_name: str, user_prompt: str, **_: object):
            ...
            if "archive/projects" in path_hint:
                return {
                    ...
                    "canonicality": "evidence",
                    "target_path": "sources/legacy/projects/sample-old.md",
                    "domain": "work",
                    "entity_refs": ["project:sample"],
                    "decision_refs": [],
                    ...
                    "quarantine_reason": None,
                    "entity_candidates": [
                        {"ref": "project:sample", "display_name": "Sample", "aliases": [], "confidence": "medium"}
                    ],
                    "atoms": [
                        {
                            "kind": "project_update",
                            "subject_ref": "project:sample",
                            "payload": {"title": "Sample", "summary": "Sample was parked."},
            ...
            return {
                ...
                "quarantine_reason": None,
                "entity_candidates": [
                    {"ref": "project:sample", "display_name": "Sample", "aliases": [], "confidence": "high"}
                ],
                "atoms": [
                    {
                        "kind": "project_update",
                        "subject_ref": "project:sample",
                        "payload": {"title": "Sample", "summary": "Sample is the active focus."},
    ...
    assert "project:sample" in payload


def test_migration_engine_preserves_legacy_time_refs_in_canonical_timeline(tmp_path: Path) -> None:
    legacy_root = tmp_path / "legacy"
    (legacy_root / "memory" / "archive" / "projects").mkdir(parents=True)
    archived_project = legacy_root / "memory" / "archive" / "projects" / "sample-old.md"
    archived_project.write_text("Sample was parked.\n", encoding="utf-8")
    ...
    project_state = load_markdown_document((output_root / "projects" / "sample" / "state.md").read_text(encoding="utf-8"))

    assert "2026-04-01: Sample was parked." in project_state.body
...
def test_migration_engine_replaces_project_state_claims_instead_of_keeping_multiple_active_claims(tmp_path: Path) -> None:
    ...
    (legacy_root / "memory" / "archive" / "projects").mkdir(parents=True)
    (legacy_root / "memory" / "daily").mkdir(parents=True)
    (legacy_root / "memory" / "archive" / "projects" / "sample-old.md").write_text(
        "Sample was parked.\n",
        encoding="utf-8",
    )
    (legacy_root / "memory" / "daily" / "2026-04-13-digest.md").write_text(
        "Sample is the active focus.\n",
        encoding="utf-8",
    )
    ...
    current = store.current_claims("project:sample")
    history = store.claim_history("project:sample")
    project_state = load_markdown_document((output_root / "projects" / "sample" / "state.md").read_text(encoding="utf-8"))

    assert len(current) == 1
    assert current[0].statement == "Sample is the active focus."
    assert any(item.status == "replaced" and item.statement == "Sample was parked." for item in history)
    assert "## Current State" in project_state.body
    assert "Sample is the active focus." in project_state.body
