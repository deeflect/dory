...
""",
    ...
    support_path = corpus_root / "projects" / "sample" / "state.md"
...
Sample remains the active project focus.
""",
        encoding="utf-8",
    )
    ...
    assert resp.sources[0] in {
        ...
        "projects/sample/state.md",
    }
    assert "wiki/projects/sample.md" in resp.sources
    assert "## Answer" in resp.artifact.body
    assert "## Evidence" in resp.artifact.body
    assert "Sample" in resp.artifact.body
    assert "wiki/projects/sample.md" in resp.artifact.sources
