...
def test_durable_hybrid_search_does_not_fallback_to_session_plane(
    ...
    engine = SearchEngine(index_root, fake_embedder)
    response = engine.search(SearchReq(query="recent Sample active focus session", mode="hybrid", k=5))
...
def test_hybrid_all_search_can_include_session_plane_for_recent_work_queries(
    ...
    engine = SearchEngine(index_root, fake_embedder)
    response = engine.search(SearchReq(query="recent Sample active focus session", mode="hybrid", corpus="all", k=5))
