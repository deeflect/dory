    ...
    class FakeClient:
        def generate_json(self, **kwargs):
            return {
                ...
                "key_facts": ["Atlas pricing settled around BYOK tiers."],
...
""",
    ...
    assert "wiki/projects/sample.md" in written
    assert "wiki/index.md" in written
    assert "wiki/hot.md" in written
    assert "wiki/log.md" in written
    target = corpus_root / "wiki" / "projects" / "sample.md"
    assert target.exists()
    content = target.read_text(encoding="utf-8")
    assert "type: wiki" in content
    assert "Sample is the active focus this week." in content
