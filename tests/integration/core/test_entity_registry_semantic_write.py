...
def test_semantic_write_resolves_new_subjects_added_after_engine_init(tmp_path) -> None:
    ...
    assert response.target_path == "projects/sample/state.md"
    assert "Sample is active." in (root / "projects" / "sample" / "state.md").read_text(encoding="utf-8")
