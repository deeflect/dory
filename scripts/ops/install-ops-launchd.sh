...
cat > "${AGENTS_DIR}/ai.dory.daily-digest-once.plist" <<PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key>
  <string>ai.dory.daily-digest-once</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/bin/env</string>
    <string>bash</string>
    <string>-lc</string>
    <string>cd "$REPO_ROOT" && uv run dory --corpus-root "$CORPUS_ROOT" --index-root "$INDEX_ROOT" ops daily-digest-once --min-age-minutes 30</string>
  </array>
  <key>StartCalendarInterval</key>
  <dict>
    <key>Hour</key><integer>2</integer>
...
launchctl unload "${AGENTS_DIR}/ai.dory.daily-digest-once.plist" >/dev/null 2>&1 || true
